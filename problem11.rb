...
01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48
  ...
  def solve()
    ...
    # 改行の置換
    source = get_source.tr("\n", " ")

    # 配列化
    source_array = source.split(" ")

    source_array.length.times {|i|
      num = search(source_array, i)
      max = num if max < num
    }
  ...
  def search(source, index)
    ...
    if y <= @@range - 4 && x >= 3
      tmp = 1
      4.times {|i|
        tmp *= source[index + (@@range*i) - i].to_i
      }
      sum = tmp if sum < tmp
    end
